# run_once_install-cli-markdown.ps1
# chezmoi 在新機器第一次 apply 時自動執行一次
# 安裝 cli-markdown（Node.js 若未安裝會自行透過 winget 安裝）

$ErrorActionPreference = "Stop"
$warnings = [System.Collections.Generic.List[string]]::new()

Write-Host "========== Bootstrap: cli-markdown ==========" -ForegroundColor Cyan
Write-Host ""
...
# 安裝 cli-markdown
# ------------------------------------------------------------
$exe = Get-Command cli-markdown -ErrorAction SilentlyContinue
if ($exe) {
    $ver = & cli-markdown --version 2>&1 | Select-Object -First 1
    Write-Host "  [skip] cli-markdown already installed ($ver)" -ForegroundColor DarkGray
} else {
    Write-Host "  Installing cli-markdown..." -ForegroundColor Green
    & $npmExe install -g cli-markdown
    if ($LASTEXITCODE -ne 0) {
        Write-Host "  [error] npm install failed (exit $LASTEXITCODE)" -ForegroundColor Red
        exit 1
    }
    Write-Host "  cli-markdown installed" -ForegroundColor Green
}
Write-Host ""
...
Write-Host "========== cli-markdown Bootstrap Complete ==========" -ForegroundColor Cyan
Write-Host ""
