...
# 安裝 Zed / Neovim 設定所需的字型（不需要系統管理員）
#
# 安裝清單：
#   - Hack Nerd Font       (Zed terminal.font_family)
...
$notoUrl = "https://fonts.google.com/download?family=Noto+Sans+TC"
...
try {
    ...
    Get-ChildItem $notoDir -Recurse -Include "*.ttf","*.otf" | ForEach-Object {
...
Write-Host "  重新啟動 Zed / VSCode 後生效" -ForegroundColor DarkGray
Write-Host ""
